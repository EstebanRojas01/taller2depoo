...
public class SandboxListas
{
    ...
    {
    	List<Integer> copiaEnteros = new ArrayList<Integer>(listaEnteros.size());
    	for(int entero:listaEnteros) {copiaEnteros.add(entero);}
        return copiaEnteros;
    }
    ...
    {
    	List<String> copiaCadenas = new ArrayList<String>(listaCadenas.size());
    	for(String cadena:listaCadenas) {copiaCadenas.add(cadena);}
        return copiaCadenas;
    }
    ...
    {
    	int mitad = listaEnteros.size() / 2;
        while (mitad > 0) {
            for (int i = mitad; i < listaEnteros.size(); i++) {
                int insercion = listaEnteros.get(i);
                int j = i;
                while (j >= mitad && listaEnteros.get(j - mitad) < insercion) {
                	listaEnteros.set(j, listaEnteros.get(j - mitad));
                    j -= mitad;
                }
                listaEnteros.set(j, insercion);
            }
            mitad /= 2;
        }
    ...
    {
        int mitad = listaCadenas.size() / 2;
        while (mitad > 0) {
            for (int i = mitad; i < listaCadenas.size(); i++) {
                String insercion = listaCadenas.get(i);
                int j = i;
                while (j >= mitad && listaCadenas.get(j - mitad).compareTo(insercion) > 0) {
                    listaCadenas.set(j, listaCadenas.get(j - mitad));
                    j -= mitad;
                }
                listaCadenas.set(j, insercion);
            }
            mitad /= 2;
        }
    }
    ...
    {
    	int contador=0;
    	for (int parametro:listaEnteros) {
    		if(parametro==valor) {contador++;}
    	}
        return contador;
    }
    ...
    {
    	int contador = 0;
    	for(String parametro:listaCadenas) {
    		if(parametro.toLowerCase().contains(cadena.toLowerCase())) {
    			contador+=1;}}
    	return contador;
    }
    ...
    {
    	listaEnteros = new ArrayList<Integer>(cantidad);
